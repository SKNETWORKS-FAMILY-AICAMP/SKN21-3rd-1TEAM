...
# C-TEAM 형사법 RAG Chatbot

본 문서는 **C-TEAM-형사법 RAG Chatbot**의 전체 아키텍처, 데이터 수집·전처리 파이프라인, 체인 구성, 성능 검증 구조를 기술합니다. 
...
- **문제점** : 초기에는 장 단위로 텍스트를 분할해 검색된 컨텍스트가 길어 검색 결과가 LLM의 최대 출력 토큰 제한을 초과해 성능 평가 진행에 어려움이 있었습니다.
- **해결** : 조 단위로 텍스트를 분할하고, 검색 결과 개수를 조정 (k=2)했으나 완벽하게 해결되지는 않았습니다. 추후 time dilation을 적용하여 해결할 예정입니다.
...
* 평가 지표 정량화 (RAGAS 등)
* 토큰 갯수 제한으로 인한 RAGAS테스트 타임오버 제어
* 프롬프트 최적화
