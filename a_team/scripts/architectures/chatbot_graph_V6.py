...
# V4에서 Generator-Critic Light 패턴 추가
# - Hybrid Retriever (Vector + BM25)
# - Query Expansion (키워드 추출, 법률 용어 매핑)
# - Generator-Critic Light (답변 평가 → 필요시 재검색)
# - 유사도 임계값 필터링 (relevance_score < 0.3 → 관련 없음)
# - 판례 웹검색 제거 (Tavily) → DB 검색만 사용
# 작성자: SKN 3-1팀 A-TEAM
# 작성일: 2026-01-08
################################################
            ...
            try:
                ...
                if reranked_docs:
                    ...
                    for i, doc in enumerate(filtered_docs, 1):
                        ...
                        print(
                            f"   [{i}] 점수: {doc.metadata.get('relevance_score', 0):.4f} | 출처: {source} | {doc.page_content[:30]}...")
